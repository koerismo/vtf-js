...
const CatRomDefaultFilter: Readonly<VFilter> = {
	radius: 2,
	kernel: make_bicubic(0.0, 0.5),
};
...
export const VFilters = {
	...
	Box:        { radius: 1, kernel: (x) => (x < 0.5 ? 1 : 0) },
	...
	NICER:      { radius: 3, kernel: filter_spp_nice },
} as const satisfies Record<string, Readonly<VFilter>>;
...
export class VImageScaler {
	...
	protected calc_coeffs(
		...
		for (let x2 = 0; x2 < size2; x2++) {
			...
			for (let i = 0; i < length; i++) pixel_coeffs[i] /= pixel_coeffs_sum;
		}
	...
	resize<T extends VPixelArray>(src: VImageData<T>, dst: VImageData<T>): VImageData<T> {
		if (src.width !== this.src_width || src.height !== this.src_height)
			throw Error(
				`VImageScaler.resize: input does not match expected dimensions! (expected ${this.src_width}x${this.src_height} but got ${src.width}x${src.height})`,
			);
		if (dst.width !== this.dest_width || dst.height !== this.dest_height)
			throw Error(
				`VImageScaler.resize: output does not match expected dimensions! (expected ${this.dest_width}x${this.dest_height} but got ${dst.width}x${dst.height})`,
			);
		if (dst.data.length !== this.dest_width * this.dest_height * 4)
			throw Error(
				`VImageScaler.resize: output data length should be ${this.dest_width * this.dest_height * 4}, got ${dst.data.length} instead!`,
			);
